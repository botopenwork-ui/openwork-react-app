export const cctpTransceiverETH = {
  ...
  features: [
    ...
    'Domain-based routing: Domain 0 for Ethereum Sepolia',
    ...
    'Job payment flow: Local CCTP → Ethereum CCTP → NOWJC',
    'Refund flow: NOWJC → Ethereum CCTP → Local CCTP',
  ...
  systemPosition: {
    description: 'CCTP Transceiver (Ethereum) sits at the Native Chain tier as the central hub for all USDC transfers in OpenWork. When users post jobs on Local chains with USDC, Local CCTP Transceivers burn USDC and send CCTP messages. This Ethereum transceiver receives attestations, verifies through Circle, and mints native USDC for NOWJC to hold as job escrow. When jobs complete or disputes resolve, this transceiver sends USDC back to Local chains by burning on Ethereum and allowing Local transceivers to mint. This eliminates bridge liquidity issues since every burn creates a mint permission.',
    diagram: `
📍 CCTP Transceiver (Ethereum) in Payment Flow
...
│  Ethereum Sepolia (Domain 0) - YOU ARE HERE             │
├─────────────────────────────────────────────────────────┤
...
✓ Direct burn/mint mechanism`
  ...
  dependencies: {
    ...
    requiredBy: [
      ...
      { 
        name: 'Native Athena (Ethereum)', 
        reason: 'Uses CCTP for dispute fee transfers and resolutions requiring USDC movement.',
        type: 'Dispute Resolution'
      },
      { 
        name: 'Local CCTP Transceivers', 
        reason: 'Source chains burn USDC, Ethereum receives. Ethereum burns for refunds, Local chains receive.',
        type: 'Payment Infrastructure'
      }
    ],
    prerequisites: [
      'Circle TokenMessenger deployed on Ethereum Sepolia',
      'Circle MessageTransmitter deployed on Ethereum Sepolia',
      'Native USDC contract deployed on Ethereum Sepolia',
      'Circle attestation API access configured',
      'Domain 0 configured for Ethereum Sepolia',
  ...
  functions: [
    {
      ...
      description: 'Burn USDC on Ethereum and initiate transfer to Local chains',
      items: [
        {
          name: 'sendFast',
          signature: 'sendFast(uint256 amount, uint32 destinationDomain, bytes32 mintRecipient, uint256 maxFee)',
          whatItDoes: 'Burns USDC on Ethereum and initiates fast transfer to destination chain via Circle CCTP.',
          ...
          parameters: [
            ...
            { name: 'destinationDomain', type: 'uint32', description: 'CCTP domain ID (0=ETH, 2=OP, 3=Ethereum, 6=Base, 7=Polygon)' },
      ...
      items: [
        {
          ...
          whatItDoes: 'Receives CCTP message with Circle attestation and mints native USDC on Ethereum.',
          whyUse: 'Called when Local chains send USDC to Ethereum. Verifies attestation and triggers USDC mint.',
          howItWorks: [
...
// 4. Submit to Ethereum CCTP Transceiver
await cctpTransceiver.receive(message, attestation);

// 5. USDC now minted on Ethereum
...
const messageHash = await cctpTransceiver.getMessageHash(message);`,
  ...
  dataFlows: [
    {
      title: 'Job Payment Flow (Local → Ethereum)',
      description: 'User posts job with USDC on Local chain, USDC transferred to Ethereum for escrow',
      steps: [
        ...
        { chain: 'OP Sepolia', action: '3. Local CCTP Transceiver.sendFast(100 USDC, Domain 0, NOWJC)' },
        ...
        { chain: 'Ethereum', action: '7. Backend calls Ethereum CCTP Transceiver.receive()' },
        { chain: 'Ethereum', action: '8. Circle MessageTransmitter verifies attestation' },
        { chain: 'Ethereum', action: '9. 100 USDC minted to NOWJC on Ethereum' },
        { chain: 'Ethereum', action: '10. NOWJC holds USDC in escrow for job' }
      ]
    },
    {
      title: 'Refund Flow (Ethereum → Local)',
      description: 'Job canceled, NOWJC refunds USDC to user on Local chain',
      steps: [
        { chain: 'Ethereum', action: '1. Job canceled, NOWJC initiates refund' },
        { chain: 'Ethereum', action: '2. NOWJC approves Ethereum CCTP Transceiver' },
        { chain: 'Ethereum', action: '3. Ethereum CCTP Transceiver.sendFast(100 USDC, Domain 2, userAddress)' },
        { chain: 'Ethereum', action: '4. Circle TokenMessenger burns 100 USDC on Ethereum' },
    ...
    },
    {
      ...
      steps: [
        ...
        { chain: 'OP Sepolia', action: '3. Local CCTP sends fee to Ethereum' },
        { chain: 'Ethereum', action: '4. Ethereum CCTP receives, mints to Native Athena' },
        { chain: 'Ethereum', action: '5. Native Athena holds fee during voting' },
        { chain: 'Ethereum', action: '6. Dispute resolves, fees distributed' },
        { chain: 'Ethereum', action: '7. If refund needed, Ethereum CCTP sends back' },
  ...
  integrationGuide: {
    example: `// CCTP Transceiver (Ethereum) Integration Example
...
console.log("USDC burned on Ethereum, attestation pending");
...
localCCTP.on('FastTransferSent', async (amount, domain, recipient, maxFee) => {
  if (domain !== 3) return; // Only interested in transfers to Ethereum
  ...
  console.log("USDC minted on Ethereum");
});
...
const CCTP_DOMAINS = {
  ...
  deployConfig: {
    ...
    postDeploy: {
      ...
      nextSteps: [
        '1. Deploy CCTPv2Transceiver with Circle contract addresses on Ethereum',
        ...
        '8. CRITICAL: This is central hub - all USDC flows through Ethereum'
      ]
    }
  },
