export const lowjcETH = {
  ...
  overview: {
    purpose: 'LOWJC on Ethereum Sepolia is the primary user interface for the OpenWork platform. Users post jobs with USDC on Ethereum, which LOWJC splits into two parallel flows: (1) Job metadata sent via Local Bridge to NOWJC on Arbitrum for storage in Genesis, and (2) USDC sent via CCTP to NOWJC for escrow. This dual-channel architecture enables cross-chain job management while keeping funds secure on Arbitrum. Users can post jobs on OP but pay workers on any supported chain (Ethereum, Polygon, Base, etc.) through NOWJC\'s cross-chain payment capabilities.',
...
1. User on Ethereum → LOWJC.postJob(100 USDC)
...
2. LOWJC splits into TWO parallel operations:
   ...
   B. USDC PATH (CCTP):
      LOWJC.sendFunds() → CCTP Transceiver (Ethereum).sendFast()
      → Burns 100 USDC on Ethereum
...
Key Benefits:
✓ Post once on Ethereum, visible everywhere
...
✓ Secure escrow on Arbitrum`
  ...
  dependencies: {
    dependsOn: [
      { 
        name: 'Local Bridge (Ethereum)', 
        reason: 'Sends job data, applications, work submissions to Native Chain via LayerZero messages.',
        type: 'Bridge Infrastructure'
      },
      { 
        name: 'CCTP Transceiver (Ethereum)', 
        reason: 'Burns USDC on Ethereum and initiates mint on Arbitrum for job escrow.',
    ...
    prerequisites: [
      'UUPS Proxy deployed on Ethereum Sepolia',
      ...
      items: [
        {
          ...
          whatItDoes: 'Creates user profile on Ethereum and syncs to Native Chain for unified identity.',
...
await lowjc.createProfile(
  ...
  { value: ethers.parseEther("0.001") } // LayerZero fee
        ...
        },
        {
          ...
          whatItDoes: 'Updates user profile IPFS hash on Ethereum and syncs to Native Chain.',
      ...
      items: [
        {
          ...
          whatItDoes: 'Posts new job on Ethereum with USDC milestones, syncs to Arbitrum for storage and escrow.',
      ...
      items: [
        {
          ...
          parameters: [
            ...
            { name: '_preferredChainDomain', type: 'uint32', description: 'CCTP domain for payments (0=Ethereum, 2=OP, 3=Arb, 6=Base)' },
            { name: '_nativeOptions', type: 'bytes', description: 'LayerZero options' }
          ],
...
const preferredChain = 2; // Ethereum Sepolia (want payment on OP)
...
await lowjc.submitWork(
  ...
  { value: ethers.parseEther("0.001") }
        ...
        },
        {
          ...
          parameters: [
            { name: '_jobId', type: 'string', description: 'Job ID' },
            { name: '_targetChainDomain', type: 'uint32', description: 'CCTP domain (0=Ethereum, 2=OP, 3=Arb, 6=Base, 7=Polygon)' },
          ...
          example: `const jobId = "10232-1";
const ethDomain = 0; // Pay on Ethereum
...
// 6. CCTP Transceiver (Ethereum).receive()
// 7. Mints 99 USDC to recipient on Ethereum
...
const jobTakerChain = 0; // Ethereum
...
// Funds released to job taker`,
  ...
  integrationGuide: {
    example: `// LOWJC (Ethereum) Integration Example
...
const preferredChain = 0; // Ethereum
...
await lowjc.releasePaymentCrossChain(
  ...
  { value: releaseFee }
    ...
    tips: [
      ...
      'Same contract on Ethereum, OP, Polygon, Base (only addresses differ)',
      ...
      'Cross-chain payments work seamlessly (post on Ethereum, pay on OP)',
