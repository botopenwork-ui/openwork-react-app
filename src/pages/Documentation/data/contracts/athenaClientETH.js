export const athenaClientETH = {
  ...
  systemPosition: {
    description: 'Athena Client sits on Local chains as the dispute/oracle interface for users. When disputes are raised, the client immediately routes USDC fees to Native Athena via CCTP (burn on Ethereum, mint on Arbitrum) while sending dispute details via LayerZero. Native Athena coordinates oracle voting, calculates rewards, and sends results back. Upon receiving finalization, Athena Client automatically calls LOWJC.resolveDispute() to execute the outcome. This architecture centralizes all fee management and oracle coordination on Arbitrum while allowing users on any chain to access dispute services.',
    diagram: `
📍 Athena Client (Ethereum) in OpenWork Architecture
...
1. User on Ethereum → Athena Client.raiseDispute(50 USDC)
...
2. Athena Client splits into TWO parallel operations:
   ...
   B. USDC FEE PATH (CCTP):
      ...
      → CCTP Transceiver (Ethereum).sendFast()
      → Burns 50 USDC on Ethereum
      ...
      items: [
        {
          ...
          whatItDoes: 'Initiates dispute on Ethereum, routes USDC fee to Native Athena, sends dispute data via LayerZero.',
...
await athenaClient.askAthena(
  ...
  integrationGuide: {
    example: `// Athena Client (Ethereum) Integration Example
...
console.log("Is Finalized:", disputeInfo.isFinalized);`,
    tips: [
      ...
      'Same contract on Ethereum, Ethereum, Polygon, Base',
