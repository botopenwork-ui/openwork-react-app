export const cctpTransceiverOP = {
  ...
  overview: {
    purpose: 'CCTP Transceiver on OP (Native Chain) facilitates native USDC transfers in OpenWork\'s payment infrastructure. When users post jobs on Local chains (OP, Ethereum, etc.) with USDC payment, Local CCTP Transceivers burn USDC and send attestations to this OP transceiver, which receives and mints USDC for NOWJC. For refunds or dispute resolutions, this transceiver sends USDC back to Local chains. Unlike bridge-wrapped tokens, CCTP transfers native USDC, eliminating liquidity risks and ensuring instant finality through Circle\'s attestation network.',
    tier: 'Native Chain (OP Sepolia)',
  ...
  features: [
    ...
    'Job payment flow: Local CCTP → OP CCTP → NOWJC',
    'Refund flow: NOWJC → OP CCTP → Local CCTP',
  ...
  systemPosition: {
    description: 'CCTP Transceiver (OP) sits at the Native Chain tier as the central hub for all USDC transfers in OpenWork. When users post jobs on Local chains with USDC, Local CCTP Transceivers burn USDC and send CCTP messages. This OP transceiver receives attestations, verifies through Circle, and mints native USDC for NOWJC to hold as job escrow. When jobs complete or disputes resolve, this transceiver sends USDC back to Local chains by burning on OP and allowing Local transceivers to mint. This eliminates bridge liquidity issues since every burn creates a mint permission.',
    diagram: `
📍 CCTP Transceiver (OP) in Payment Flow
  ...
  functions: [
    {
      ...
      description: 'Burn USDC on OP and initiate transfer to Local chains',
      items: [
        {
          name: 'sendFast',
          signature: 'sendFast(uint256 amount, uint32 destinationDomain, bytes32 mintRecipient, uint256 maxFee)',
          whatItDoes: 'Burns USDC on OP and initiates fast transfer to destination chain via Circle CCTP.',
          ...
          parameters: [
            ...
            { name: 'destinationDomain', type: 'uint32', description: 'CCTP domain ID (0=ETH, 2=OP, 3=OP, 6=Base, 7=Polygon)' },
      ...
      items: [
        {
          ...
          whatItDoes: 'Receives CCTP message with Circle attestation and mints native USDC on OP.',
          whyUse: 'Called when Local chains send USDC to OP. Verifies attestation and triggers USDC mint.',
          howItWorks: [
...
// 4. Submit to OP CCTP Transceiver
await cctpTransceiver.receive(message, attestation);

// 5. USDC now minted on OP
...
const messageHash = await cctpTransceiver.getMessageHash(message);`,
  ...
  dataFlows: [
    {
      title: 'Job Payment Flow (Local → OP)',
      description: 'User posts job with USDC on Local chain, USDC transferred to OP for escrow',
      steps: [
        ...
        { chain: 'OP', action: '7. Backend calls OP CCTP Transceiver.receive()' },
        { chain: 'OP', action: '8. Circle MessageTransmitter verifies attestation' },
        { chain: 'OP', action: '9. 100 USDC minted to NOWJC on OP' },
        { chain: 'OP', action: '10. NOWJC holds USDC in escrow for job' }
      ]
    },
    {
      title: 'Refund Flow (OP → Local)',
      description: 'Job canceled, NOWJC refunds USDC to user on Local chain',
      steps: [
        { chain: 'OP', action: '1. Job canceled, NOWJC initiates refund' },
        { chain: 'OP', action: '2. NOWJC approves OP CCTP Transceiver' },
        { chain: 'OP', action: '3. OP CCTP Transceiver.sendFast(100 USDC, Domain 2, userAddress)' },
        { chain: 'OP', action: '4. Circle TokenMessenger burns 100 USDC on OP' },
    ...
    },
    {
      ...
      steps: [
        ...
        { chain: 'OP Sepolia', action: '3. Local CCTP sends fee to OP' },
        { chain: 'OP', action: '4. OP CCTP receives, mints to Native Athena' },
        { chain: 'OP', action: '5. Native Athena holds fee during voting' },
        { chain: 'OP', action: '6. Dispute resolves, fees distributed' },
        { chain: 'OP', action: '7. If refund needed, OP CCTP sends back' },
  ...
  integrationGuide: {
    example: `// CCTP Transceiver (OP) Integration Example
...
console.log("USDC burned on OP, attestation pending");
...
localCCTP.on('FastTransferSent', async (amount, domain, recipient, maxFee) => {
  if (domain !== 3) return; // Only interested in transfers to OP
...
const CCTP_DOMAINS = {
  ...
  deployConfig: {
    ...
    postDeploy: {
      ...
      nextSteps: [
        '1. Deploy CCTPv2Transceiver with Circle contract addresses on OP',
        ...
        '8. CRITICAL: This is central hub - all USDC flows through OP'
      ]
    }
  },
