export const localBridgeOP = {
  ...
  features: [
    'User gateway: Connects OP users to Native execution layer',
    ...
    'Chain identification: Stores thisLocalChainEid (40232 for OP Sepolia)',
  ...
  systemPosition: {
    description: 'Local Bridge (OP) sits at the user-facing tier of OpenWork\'s three-tier architecture. Users on OP Sepolia interact with LOWJC (job posting) and Athena Client (dispute raising), which send cross-chain messages through this bridge. The bridge forwards job operations to Native Bridge on Arbitrum, which routes them to NOWJC and Native Athena for execution. When disputes are finalized on Native Chain, Native Bridge sends results back through this Local Bridge to Athena Client on OP. Main DAO can also send upgrade commands to this bridge to upgrade Athena Client or LOWJC. This bridge is one instance of an identical contract deployed on multiple Local Chains.',
    diagram: `
📍 Local Bridge (OP) in Three-Tier Architecture

┌─────────────────────────────────────────────────────────┐
│  TIER 1: LOCAL CHAINS (User-Facing)                     │
│  OP Sepolia ⭐ (YOU ARE HERE)                           │
├─────────────────────────────────────────────────────────┤
│  • LOWJC (job interface)                                │
...
1. Job Posting (User → Execution):
   User → LOWJC (OP) → Local Bridge (OP) → Native Bridge → NOWJC

2. Dispute Raising (User → Resolution):
   User → Athena Client (OP) → Local Bridge (OP) → Native Bridge → Native Athena

3. Dispute Finalization (Result → User):
   Native Athena → Native Bridge → Local Bridge (OP) → Athena Client (OP)

4. Upgrade Command (Governance → Local):
   Main DAO → Main Bridge → Local Bridge (OP) → Athena Client (OP)
...
5. Multi-Chain Deployment:
  ...
  dependencies: {
    ...
    prerequisites: [
      'LayerZero V2 Endpoint deployed on OP Sepolia',
      'Native Chain EID configured (Arbitrum Sepolia)',
      'Main Chain EID configured (Base Sepolia)',
      'thisLocalChainEid set to 40232 (OP Sepolia)',
...
// → Routes to Athena Client.handleFinalizeDisputeWithVotes()`,
        ...
        },
        {
          ...
          gasEstimate: '~95K gas',
          example: `// Main DAO upgrades Athena Client on OP
...
await localBridge.authorizeContract(athenaClientAddress, true);`,
        ...
        },
        {
          ...
          parameters: [
            ...
            { name: '_thisLocalChainEid', type: 'uint32', description: 'This Local Chain EID (40232 for OP)' }
          ],
          accessControl: 'onlyOwner',
  ...
  dataFlows: [
    {
      ...
      description: 'User posts job on OP, sent to Native for execution',
      steps: [
        { chain: 'OP Sepolia', action: '1. User calls LOWJC.postJob() with payment' },
        { chain: 'OP Sepolia', action: '2. USDC sent to Native via CCTP' },
        { chain: 'OP Sepolia', action: '3. LOWJC calls Local Bridge.sendToNativeChain()' },
    ...
    {
      title: 'Dispute Flow',
      description: 'User raises dispute on OP, resolved on Native, result returned',
      steps: [
        { chain: 'OP Sepolia', action: '1. User calls Athena Client.raiseDispute()' },
        { chain: 'OP Sepolia', action: '2. Athena Client calls Local Bridge.sendToNativeChain()' },
        ...
        { chain: 'Arbitrum', action: '7. Native Bridge sends result to Local Bridge (OP)' },
        { chain: 'OP Sepolia', action: '8. Local Bridge._lzReceive() receives' },
        { chain: 'OP Sepolia', action: '9. Routes to Athena Client.handleFinalizeDisputeWithVotes()' },
        { chain: 'OP Sepolia', action: '10. Dispute status updated on OP' }
      ]
    },
    {
      title: 'Upgrade Flow',
      description: 'Main DAO upgrades Athena Client on OP',
      steps: [
        { chain: 'Base/Ethereum', action: '1. Main DAO proposal passes' },
        { chain: 'Base/Ethereum', action: '2. Main Bridge.sendUpgradeCommand() to OP' },
        { chain: 'OP Sepolia', action: '3. Local Bridge._lzReceive() receives upgrade' },
        { chain: 'OP Sepolia', action: '4. Routes to _handleUpgradeMessage()' },
        { chain: 'OP Sepolia', action: '5. Calls Athena Client.upgradeFromDAO()' },
        { chain: 'OP Sepolia', action: '6. Athena Client upgrades to new implementation' },
        { chain: 'OP Sepolia', action: '7. New Athena Client features active' }
      ]
    }
  ],
  
  integrationGuide: {
    example: `// Local Bridge (OP) Integration Example
...
// Native Athena → Native Bridge → Local Bridge → Athena Client`,
    tips: [
      'Local Bridge is user gateway - connects OP to Native/Main',
      ...
      'thisLocalChainEid must match actual chain (40232 for OP)',
  ...
  deployConfig: {
    ...
    constructor: [
      {
        name: '_endpoint',
        type: 'address',
        description: 'LayerZero V2 Endpoint address on OP Sepolia',
      ...
      },
      {
        ...
        type: 'uint32',
        description: 'LayerZero EID for THIS chain (OP Sepolia: 40232)',
        placeholder: '40232'
      }
    ],
    networks: {
    ...
    postDeploy: {
      ...
      nextSteps: [
        ...
        '   - This Local Chain EID: 40232 (OP Sepolia)',
        '2. Set Local Chain contract addresses:',
        '   - setAthenaClientContract(athenaClientOPAddress)',
        '   - setLowjcContract(lowjcOPAddress)',
        '3. Authorize Local contracts to use bridge:',
        '   - authorizeContract(athenaClientOPAddress, true)',
        '   - authorizeContract(lowjcOPAddress, true)',
        ...
        '9. Verify contract on OP Sepolia Etherscan',
  ...
  ,
  code: `// Same implementation as other Local Bridges - see: contracts/openwork-full-contract-suite-layerzero+CCTP 2 Dec/local-bridge.sol
