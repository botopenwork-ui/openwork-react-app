...
contract NativeArbOpenWorkJobContractTestnet is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    /**
     * @dev TESTNET ONLY — sets a profile directly without NOWJC sync.
     * Needed because nowjc-v2 createProfile is commented out (Genesis lacks setProfile).
     * This function does NOT exist on the mainnet contract.
     */
    function adminSetProfile(address user) external onlyOwner {
        require(!hasProfile[user], "Profile already exists");
        profiles[user] = Profile({
            userAddress: user,
            ipfsHash: "testnet-profile",
            referrerAddress: address(0),
            portfolioHashes: new string[](0)
        });
        hasProfile[user] = true;
        emit ProfileCreated(user, "testnet-profile", address(0));
    }
    ...
    function createProfile(
        ...
        // TESTNET: Skip NOWJC sync (createProfile commented out in nowjc-v2)
