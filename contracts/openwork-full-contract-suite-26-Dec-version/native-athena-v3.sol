    ...
    UUPSUpgradeable
{
    ...
    address public athenaClient; // NativeArbAthenaClient — authorized to call handle* functions natively
    ...
    function updateOracleActiveStatus(string memory _oracleName) public {
        ...
        for (uint256 i = 0; i < members.length; i++) {
            if (canVote(members[i])) activeCount++;
        }
    ...
    function isOracleActive(string memory _oracleName) public view returns (bool) {
        // Compute on-the-fly (no genesis cache required)
        IOpenworkGenesis.Oracle memory oracle = genesis.getOracle(_oracleName);
        if (oracle.members.length == 0) return false;
        uint256 activeCount = 0;
        for (uint256 i = 0; i < oracle.members.length; i++) {
            if (canVote(oracle.members[i])) {
                activeCount++;
            }
        }
        return activeCount >= minOracleMembers;
    }
    ...
    function handleAskAthena(address applicant, string memory description, string memory hash, string memory targetOracle, string memory fees) external {
        require(msg.sender == address(bridge) || msg.sender == athenaClient, "Only bridge or athenaClient");
    ...
    function vote(
        ...
        if (voteWeight == 0) voteWeight = 1; // testnet: allow voting with weight=1 if no earned tokens
    ...
    function _parseJobIdForChainDomain(string memory _jobId) internal pure returns (uint32) {
        ...
        // Supports both LayerZero EIDs and real chain IDs (bug-fix: LOWJC uses chainId, not EID)
        if (eid == 40161 || eid == 11155111) return 0; // Ethereum Sepolia
        else if (eid == 1) return 0;                    // Ethereum mainnet
        else if (eid == 40232 || eid == 11155420) return 2; // OP Sepolia
        else if (eid == 10) return 2;                   // OP mainnet
        else if (eid == 40231 || eid == 421614) return 3; // Arbitrum Sepolia
        else if (eid == 30110 || eid == 42161) return 3;  // Arbitrum mainnet
