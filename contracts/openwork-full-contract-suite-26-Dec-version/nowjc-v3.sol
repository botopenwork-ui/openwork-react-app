    ...
    UUPSUpgradeable
{
    ...
    mapping(address => bool) public hasNowjcProfile; // local profile registry for native Arb users
  ...
  /*  function getPlatformBandInfo() external view returns (
    ...
    function createProfile(address _user, string memory _ipfsHash, address _referrerAddress) external {
        require(authorizedContracts[msg.sender], "!");
        if (hasNowjcProfile[_user]) return; // idempotent — safe to call multiple times
        hasNowjcProfile[_user] = true;
  ...
  /*  function getApplication(string memory _jobId, uint256 _applicationId) public view returns (Application memory) {
    ...
    //     usdcToken.safeTransfer(job.selectedApplicant, _amount);
    ...
    function releasePayment(string memory _jobId) external {
        require(authorizedContracts[msg.sender] || msg.sender == bridge, "!");
  ...
  /*  function getJobStatus(string memory _jobId) external view returns (JobStatus) {
    ...
    function releaseDisputedFunds(
        ...
        require(msg.sender == nativeAthena, "!Athena");
