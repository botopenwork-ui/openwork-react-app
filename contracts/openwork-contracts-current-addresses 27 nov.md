...
**GenesisReaderHelper v2** (November 26, 2025):
- **Purpose**: Efficient batch data retrieval without requiring job/dispute IDs
- **Key Feature**: Dynamically discovers all disputes including multi-disputes (jobId-1, jobId-2, etc.)
- **Functions**: `getAllDisputeIds()`, `getActiveDisputes()`, `getActiveSkillApplications()`, `getActiveAskAthenaApplications()`
- **Performance**: Reduces RPC calls from 100+ to 1 for dispute loading
- **Frontend Guide**: `references/UI helper/genesis-reader-helper-frontend-guide.md`
- **Verification GUID**: `genwt1tz9pippwnbrdvazfhbkk2nhc9rvw93cxy9njsu3jrqbm`
