...
/// @title NativeLZOpenworkBridge V2
...
contract NativeLZOpenworkBridgeV2 is OAppSender, OAppReceiver {
    ...
    /// @param _options LayerZero messaging options
    /// @custom:fix V2-REFUND-FIX: Uses `user` as refund address so excess ETH goes back to caller, not RewardsContract
    function sendSyncVotingPower(
        ...
        _lzSend(
            ...
            _options,
            MessagingFee(msg.value, 0),
            payable(user)  // V2 FIX: Refund goes to user, not msg.sender (RewardsContract)
        );
