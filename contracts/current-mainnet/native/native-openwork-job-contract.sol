    ...
    UUPSUpgradeable
{
  ...
  /*  function getApplication(string memory _jobId, uint256 _applicationId) public view returns (Application memory) {
    ...
    function releasePaymentCrossChain(
        ...
        // Verify we have sufficient funds (trust _amount from LOWJC's per-job tracking)
        uint256 actualBalance = usdcToken.balanceOf(address(this));
        require(actualBalance >= _amount, "Insufficient contract balance");

        // Calculate commission on the per-job amount (not total balance)
    ...
    /// @return actualBalance Contract's USDC balance
    /// @return netAmount Amount after commission
    /// @return commission Commission amount
    function _validateAndCalculatePayment(string memory _jobId) internal view returns (
        ...
        // Validate balance with CCTP fee tolerance (0.01%)
        ...
        commission = calculateCommission(actualBalance);
        require(actualBalance > commission, "Balance insufficient for commission");
        netAmount = actualBalance - commission;
    }
