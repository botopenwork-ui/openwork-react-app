...
/**
 * @title NativeArbOpenWorkJobContract (v2)
 ...
 *
 * v2 changes:
 *  - Profile read/write delegated to ProfileManager contract via IProfileManager
 *  - `profiles` and `hasProfile` mappings kept for storage layout preservation (unused)
 *  - `profileManager` address added at end of state variables
 *  - `getProfile` removed (callers should query ProfileManager directly)
 *
