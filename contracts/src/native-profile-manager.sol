    ...
    UUPSUpgradeable
{
    ...
    function rate(
        ...
        require(_userToRate != address(0), "Invalid address");

        // Authorization is validated by the bridge (LOWJC) before this call —
        // ProfileGenesis is profile-only and does not store job data.
        // We trust the bridge's auth check and just record the rating.
