...
contract NativeOpenworkGenesis is Initializable, UUPSUpgradeable {
    ...
    // ==================== PROFILE STORAGE (added for NOWJC v3 compatibility) ====================
    ...
    function setProfile(address user, string memory ipfsHash, address referrer) external onlyAuthorized {
        if (!_hasProfile[user]) {
            _hasProfile[user] = true;
        }
        _profiles[user].userAddress = user;
        _profiles[user].ipfsHash = ipfsHash;
        _profiles[user].referrerAddress = referrer;
    }
    ...
    function addPortfolio(address user, string memory portfolioHash) external onlyAuthorized {
        _profiles[user].portfolioHashes.push(portfolioHash);
    }

    function updateProfileIpfsHash(address user, string memory newIpfsHash) external onlyAuthorized {
        _profiles[user].ipfsHash = newIpfsHash;
    }

    function updatePortfolioItem(address user, uint256 index, string memory newPortfolioHash) external onlyAuthorized {
        require(index < _profiles[user].portfolioHashes.length, "Index out of bounds");
        _profiles[user].portfolioHashes[index] = newPortfolioHash;
    }

    function removePortfolioItem(address user, uint256 index) external onlyAuthorized {
        uint256 len = _profiles[user].portfolioHashes.length;
        require(index < len, "Index out of bounds");
        _profiles[user].portfolioHashes[index] = _profiles[user].portfolioHashes[len - 1];
        _profiles[user].portfolioHashes.pop();
    }
    
    // ==================== RATING STORAGE (added for NOWJC v3 compatibility) ====================
    
    mapping(string => mapping(address => uint256)) private _jobRatings;
    mapping(address => uint256[]) private _userRatings;
    
    function setJobRating(string memory jobId, address user, uint256 rating) external onlyAuthorized {
        _jobRatings[jobId][user] = rating;
    ...
    // ==================== GOVERNANCE BAND STUB (for NOWJC v3 compatibility) ====================
    
    function getUserGovernanceActionsInBand(address /* user */, uint256 /* band */) external pure returns (uint256) {
        return 0;
    }
    
    function setOracleActiveStatus(string memory, bool) external onlyAuthorized {
        // stub — oracle active status computed on-the-fly by NativeAthena
    }
    ...
    function memberLastActivity(address) external pure returns (uint256) {
        return 0; // stub
    }
