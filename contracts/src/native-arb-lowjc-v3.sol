...
/**
 * @title NativeArbOpenWorkJobContract (v3)
 * @notice Native Arbitrum version of LOWJC — v2 routes all profile operations to ProfileManager.
 *
 ...
 *
 * v3 changes:
 *  - startJob validates appId exists before locking funds
 *  - emergencyWithdrawUSDC for owner to recover stuck USDC
 *  - resolveDispute now accepts NativeAthena as caller (in addition to athenaClientContract)
 *  - `nativeAthena` address added at end of state variables
 *
 ...
 *  4. Call setProfileManager(profileManagerAddress)
 */
    ...
    UUPSUpgradeable
{
    ...
    // ==================== STATE VARIABLES ====================
    // NOTE: Storage layout preserved for upgrades — do NOT reorder.

    mapping(address => Profile) public profiles;       // unused in v2 (kept for layout)
    mapping(address => bool) public hasProfile;        // unused in v2 (kept for layout)
    ...
    // v2: ProfileManager — added at end to preserve prior storage layout
    address public profileManager;

    // v3: NativeAthena — allows NativeAthena to call resolveDispute
    ...
    function setProfileManager(address _profileManager) external onlyOwner {
        require(_profileManager != address(0), "Invalid address");
    ...
    function createProfile(
        ...
        IProfileManager(profileManager).createProfile(msg.sender, _ipfsHash, _referrerAddress);
    ...
    function updateProfile(string memory _newIpfsHash) external nonReentrant {
        require(profileManager != address(0), "ProfileManager not set");
        IProfileManager(profileManager).updateProfile(msg.sender, _newIpfsHash);
    ...
    function applyToJob(
        ...
        require(profileManager != address(0) && IProfileManager(profileManager).hasProfile(msg.sender), "Must have profile");
    ...
    function startJob(
        ...
        require(jobApplications[_jobId][_appId].applicant != address(0), "Invalid application");
    ...
    function rate(
        ...
        if (profileManager != address(0)) {
            IProfileManager(profileManager).rate(msg.sender, _jobId, _userToRate, _rating);
        }
    ...
    function resolveDispute(string memory _jobId, bool _jobGiverWins) external {
        require(msg.sender == athenaClientContract || msg.sender == nativeAthena, "Only Athena");
    ...
    function emergencyWithdrawUSDCTo(address _to, uint256 _amount) external onlyOwner {
