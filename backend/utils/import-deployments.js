...
async function importToDatabase(deployments) {
  ...
  for (const dep of deployments) {
    try {
      ...
      if (dep.isCurrent) {
        ...
        `, dep.contractId, dep.networkName, dep.address);
      }
  ...
  const total = await db.get(`
    SELECT COUNT(*) as count FROM deployments WHERE is_current = true
  `);
...
async function main() {
  ...
  try {
    ...
    // Import to database
    await importToDatabase(deployments);
